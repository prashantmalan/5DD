...
export interface ContextSource {
  name: string;
  level: 1 | 2 | 3 | 4;  // disclosure level
  tokenCost: number;
  content: string;
}
...
export type ContextSourceType =
  | 'logs' | 'git' | 'jira' | 'confluence' | 'cicd'
  | 'azure' | 'databricks' | 'terraform' | 'database' | 'api';

// Keywords that signal which context sources to include automatically
const SOURCE_TRIGGERS: Record<ContextSourceType, RegExp[]> = {
...
export class ContextBuilder {
  ...
  private integrations: Map<ContextSourceType, IntegrationProvider> = new Map();

  constructor(logMonitor: LogMonitor, gitMonitor: GitMonitor) {
    this.logMonitor = logMonitor;
    this.gitMonitor = gitMonitor;
  }

  registerIntegration(type: ContextSourceType, provider: IntegrationProvider): void {
  ...
  async build(request: ContextRequest): Promise<BuiltContext> {
    ...
    const relevantSources = this.detectRelevantSources(
      request.userMessage,
      request.include,
      request.exclude
    );
    ...
    if (level >= 2) {
      for (const sourceType of relevantSources) {
        if (sourceType === 'logs' || sourceType === 'git') continue;
   ...
   */
  toSystemPromptBlock(ctx: BuiltContext): string {
  ...
  private detectRelevantSources(
    message: string,
    include?: ContextSourceType[],
    exclude?: ContextSourceType[]
  ): Set<ContextSourceType> {
    const relevant = new Set<ContextSourceType>();

    if (include) {
      for (const s of include) relevant.add(s);
      return relevant;
    }
    ...
    for (const [source, patterns] of Object.entries(SOURCE_TRIGGERS)) {
      for (const pattern of patterns) {
        if (pattern.test(message)) {
          relevant.add(source as ContextSourceType);
          break;
        }
      }
    }
    ...
    if (exclude) {
      for (const s of exclude) relevant.delete(s);
    }

    return relevant;
  }

  private buildFollowUpQuestion(source: ContextSourceType): string {
    const questions: Record<ContextSourceType, string> = {
...
export interface IntegrationProvider {
  getMinimalContext(query: string, level: number): Promise<string>;
}
